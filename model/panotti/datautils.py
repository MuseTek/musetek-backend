...
def build_free_sounds_dataset(sample_mel_path,labels_path,path="Preproc/Train/", load_frac=1.0, batch_size=16, tile=False):

    mel_dims = get_sample_dimensions(sample_mel_path)  # get dims of sample data file
    pdb.set_trace() 
    if (tile):
        ldims = list(mel_dims)
        ldims[3] = 3
    ...
    labels_dict = pickle.load(open(labels_path,"rb"))

    sample_arr = next(iter(labels_dict.values()))
    sample_arr_len = sample_arr.shape[0]
    #pdb.set_trace()
    spectrogram_file_list = getListOfFiles(path)
    first_go = True
    while True:
        ...
        #load_count = 0
        #files_to_retrieve is a string if batch_size =1 so much change to list
        if(batch_size==1):
            file_string = files_to_retrieve
            files_to_retrieve = [file_string]
            #files_to_retrieve[0] = file_string
        
        for spect in files_to_retrieve:
            #pdb.set_trace()
            spect_path = path+'/'+spect
            if(not spect_path.endswith('npz')):
                continue
            ...
            try:
                this_Y = labels_dict[spect[:-8]+'.loopdata']
                #pdb.set_trace()
            except:
                continue
            # pdb.set_trace()
            ...
            if (load_count >= batch_size):   # Abort loading files after last even multiple of batch size
                break
        #verify again that the load count actually reached the required size
        if(load_count<batch_size):
            first_go =False
            continue
        first_go=True
        yield X,Y
